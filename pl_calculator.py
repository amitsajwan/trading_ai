"""P&L calculator for multi-leg option/option-like positions.

Features:
 - Accepts any number of legs (buy/sell, entry price, qty)
 - Prompts for current LTP per leg (or tries to fetch via Kite Connect if credentials.json is present)
 - Computes entry credit, current cost and P&L

Usage:
 - Optionally create `credentials.json` using `generate_session.py` to enable API LTP fetching.
 - Run: python pl_calculator.py
"""
import json
import os
