...
def test_store_ohlc_respects_limit():
    ...
    bars = [
        OHLCBar("BANKNIFTY26JANFUT", "1m", 1, 2, 0.5, 1.5, 10, base_time),
        OHLCBar("BANKNIFTY26JANFUT", "1m", 2, 3, 1.5, 2.5, 20, base_time + timedelta(minutes=1)),
        OHLCBar("BANKNIFTY26JANFUT", "1m", 3, 4, 2.5, 3.5, 30, base_time + timedelta(minutes=2)),
    ]

    for bar in bars:
        store.store_ohlc(bar)

    result = list(store.get_ohlc("BANKNIFTY26JANFUT", "1m"))
    assert len(result) == 2
