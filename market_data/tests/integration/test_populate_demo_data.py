...
import pytest
# Skip integration test if optional core kernel package is not available
pytest.importorskip('core_kernel.database')
from unittest.mock import AsyncMock, MagicMock, patch

from market_data.tools.populate_demo_data import populate_historical_data
...
class TestPopulateDemoData:
    ...
    @patch("market_data.tools.populate_demo_data.BinanceSpotFetcher")
    @patch("market_data.tools.populate_demo_data.requests.get")
    @patch("market_data.tools.populate_demo_data.get_mongo_client")
    @patch("market_data.tools.populate_demo_data.get_collection")
    async def test_populate_historical_data_success(self, mock_get_collection, mock_get_client, mock_requests_get, mock_fetcher_class):
        ...
        # Mock MarketMemory
        with patch("market_data.tools.populate_demo_data.MarketMemory") as mock_mm_class:
            mock_mm = MagicMock()
    ...
    async def test_populate_crypto_data_success(self, mock_get_collection, mock_get_client, mock_requests_get, mock_fetcher_class):
        ...
        with patch("market_data.tools.populate_demo_data.CryptoDataFeed") as mock_crypto_class:
            mock_crypto = MagicMock()
            mock_crypto_class.return_value = mock_crypto

            # Import and run the crypto population function if it exists
            try:
                from market_data.tools.populate_demo_data import populate_crypto_data
