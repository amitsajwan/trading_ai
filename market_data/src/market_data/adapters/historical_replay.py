...
def _set_system_virtual_time(timestamp: datetime):
    ...
    try:
        ...
        logger.info(f"Set system virtual time to: {timestamp}")
...
class HistoricalDataReplay(MarketIngestion):
    ...
    def __init__(self, store: MarketStore, data_source: str = "synthetic", rebase: bool = False, rebase_to: Optional[datetime] = None, start_date: Optional[datetime] = None):
        ...
        Args:
            ...
            rebase: if True, shift timestamps so data appears "now" instead of changing system virtual time
            rebase_to: explicit datetime to use as the target 'now' for rebase
            start_date: datetime to start synthetic data generation from (defaults to 1 hour ago)
        """
        self.store = store
    ...
    def _generate_synthetic_data(self) -> List[Dict[str, Any]]:
        ...
        for i in range(60):
            ...
            # Note: Individual ticks are stored via store.store_tick() calls
            # during replay loop, not here during data generation
        ...
        try:
            ...
            if self.rebase:
                if self.rebase_offset is None:
                    ...
                    # Convert target to naive datetime for consistent arithmetic
                    if target.tzinfo is not None:
                        target = target.replace(tzinfo=None)
                    self.rebase_offset = target - first_ts
                timestamp = timestamp + self.rebase_offset
            else:
                # Set system virtual time if not rebasing and env indicates it
