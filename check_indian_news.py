...
# Query for recent EODHD news (Indian market news)
cutoff_time = datetime.now() - timedelta(hours=1)
indian_news = list(news_collection.find(
    {'event_type': 'NEWS', 'source': 'eodhd', 'timestamp': {'$gte': cutoff_time.isoformat()}},
    {'_id': 0}
).sort('timestamp', -1).limit(3))

print(f'Found {len(indian_news)} recent EODHD (Indian market) news items')
if indian_news:
