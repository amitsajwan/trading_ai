...
# Setup Python path to import data_niftybank module
...
from data_niftybank.contracts import MarketTick, OHLCBar, NewsItem, MacroIndicator
...
def sample_news_items():
    ...
    return [
        NewsItem(
            ...
            content="NIFTY index showed strong gains today...",
...
def sample_macro_indicators():
    """Sample macroeconomic indicators for testing."""
    base_time = datetime.now()
    return [
        MacroIndicator(
            name="CPI Inflation",
...
class MockMacroDataFetcher:
    """Mock macro data fetcher for testing."""

    async def get_inflation_data(self):
        """Get mock inflation data."""
        return [
            {
                "value": 5.2,
                "date": datetime.now()
    ...
    async def get_rbi_data(self, indicator):
        """Get mock RBI data."""
        return [
            {
                "value": 6.5,
